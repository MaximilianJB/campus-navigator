...
export interface Feature<G extends PointGeometry | PolygonGeometry = PointGeometry | PolygonGeometry> {
    type: "Feature";
    geometry: G;
...
export const validateGeoJSON = (data: unknown, fileSize: number): string[] => {
    ...
    for (const feature of features) {
        if (feature?.type !== "Feature") {
            errs.push('Invalid Feature: Each item in "features" must have type "Feature".');
            continue;
        }
        if (!feature.geometry) {
        ...
        if (geometryType === "Polygon") {
            ...
            if (!props?.name || typeof props.name !== 'string' || (props.name as string).trim() === '') {
                errs.push(`Invalid Polygon Feature: Missing or empty "properties.name".`);
            }
            // Check coordinates structure
            if (!Array.isArray(polygonFeature.geometry.coordinates) || polygonFeature.geometry.coordinates.length === 0) {
                errs.push(`Invalid Polygon Geometry: "coordinates" must be a non-empty array of rings.`);
            } else {
                // Basic linear ring check (doesn't check self-intersection)
                if (!isLinearRing(polygonFeature.geometry.coordinates[0])) {
                    errs.push(`Invalid Polygon Geometry: Outer ring must be a valid LinearRing (>=4 points, closed, finite coordinates).`);
                }
            ...
            if (!isValidCoordinate(pointFeature.geometry.coordinates)) {
                errs.push(`Invalid Point Geometry: Coordinates must be [longitude, latitude] with finite numbers.`);
            } else {
            ...
            errs.push(`Unsupported Feature Type: "${geometryType}". Only "Polygon" and "Point" are supported.`);
        }
    }
